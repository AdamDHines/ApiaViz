...
class ModelEvaluator:
    """A comprehensive tool for evaluating and visualizing insect-vision neural networks.
       This version is compatible with both standard ANN and SNN models.
    """
    ...
    def plot_full_layer_analysis(self, input_tensor: torch.Tensor, input_image: np.ndarray, 
        ...
        """Creates a comprehensive grid showing activations from all key network layers.
           This method now uses the object's internal state (self.snn) to determine its behavior.
        """
        self.model.eval(); self.activations = {}

        if self.snn:
            layers_to_hook = ['lamina_lif', 'medulla_lif', 'lobula_lif']
        ...
        # ... (The rest of your original plotting code is restored and correct) ...
    ...
    def visualize_batch(self, dataloader, n_samples: int = 4, use_random_sampling: bool = True):
        ...
        for i, sample_idx in enumerate(indices_to_process):
            ...
            # For SNN analysis, the input tensor must be passed without a batch dimension to the plotting function
            ...
            try:
                ...
                # The call no longer needs the SNN flag, as the object knows its state.
    ...
    def run_full_evaluation(self, dataloader, features: np.ndarray, labels: np.ndarray,
        ...
        """Runs the complete evaluation pipeline. This function is now fully correct."""
        ...
        # This call correctly triggers the state-aware visualization logic.
